...
sub cut_fasta_advanced{
	...
	my $Total_len=0;
	open IN,(($infile=~/\.gz$/)? "gzip -dc $infile|" : $infile) or die "can not open $infile:$!";
	$/=">";<IN>;$/="\n";
	while(<IN>) {
			chomp;
			my $head=$_;
...
sub cut_fasta{
	...
	chomp $file_name;
	my $out_dir = (defined $Outdir) ? "$Outdir/$file_name.cut" : "./$file_name.cut";
	my $total_num = `less $infile|grep -c '>'`;  chomp $total_num;
	...
	my $num_loop=0;
	open IN, (($infile=~/\.gz$/)? "gzip -dc $infile|" : $infile) || die "$infile can be open\n";
	$/=">"; <IN>; $/="\n";
	while (<IN>) {
		my $title=">".$_;
		$/=">";
