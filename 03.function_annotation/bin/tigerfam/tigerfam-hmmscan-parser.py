...
with open(tmpfile) as f:
	print('SeqID\tSeqLength\tAccession\tHMMProfile\tProfileLength\tEValue\tProfileStart\tProfileEnd\tSeqStart\tSeqEnd\tCoverage\tDescription')
	for line in f:
		...
		if float(row[4]) <= eval and float(row[-1]) >= coverage:
			...
			if row[9] in pfam_dsc:
				new_row.append(pfam_dsc[row[9]])
			else:
				new_row.append('-')
			new_new_row = []
			for j in new_row:
